...
export type LatestTrackedOutcome = {
  reportId: number;
  horizonDays: number;
  reportDate: string;
...
export async function fetchSavedReports(
  ...
  symbol?: string,
  sourceUrl?: string
): Promise<ApiResponse<SavedReportListItem[]>> {
...
export async function fetchLatestTrackedOutcome(
  symbol: string,
  country: "IN" | "US",
  horizonDays = 90
): Promise<ApiResponse<LatestTrackedOutcome | null>> {
  const params = new URLSearchParams({
    symbol: symbol.trim().toUpperCase(),
    country,
    horizonDays: String(horizonDays),
  });
  return fetchJSON<LatestTrackedOutcome | null>(`/api/reports/outcomes/latest?${params.toString()}`);
}
...
export async function fetchThesisMemory(symbol: string, country: "IN" | "US"): Promise<ApiResponse<ThesisMemoryData>> {
  ...
  try {
    return await fetchJSON<ThesisMemoryData>(`/api/company/thesis?${params.toString()}`);
  } catch (error: any) {
    const message = String(error?.message || "");
    if (message === "thesis_not_found") {
      return { success: false, error: "thesis_not_found" };
    }
    throw error;
  }
}
